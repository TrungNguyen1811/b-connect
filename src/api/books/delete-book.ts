...
export async function deleteBook(id: string) {
  return await authAxiosClient.delete(`/products/delete-book?productId=${id}`).then((res) => res.data)
}
...
export async function removeBookFromBookGroup(productId: string, bookGroupId: string) {
  return await authAxiosClient
    .delete(`/products/book-group/remove-book-from-book-group?productId=${productId}&bookGroupId=${bookGroupId}`)
    .then((res) => res.data)
}
